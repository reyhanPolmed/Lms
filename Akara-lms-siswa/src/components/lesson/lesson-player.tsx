      ...
      <div className="surface-card group overflow-hidden p-5 sm:p-6">
        <div className="overflow-hidden rounded-[22px] border border-slate-200 bg-slate-100">
          {lesson.contentType === "video" ? (
            ...
            <div className="flex aspect-video items-end bg-[linear-gradient(145deg,#eef2f7_0%,#f8fafc_44%,#ffffff_100%)] p-6">
              <div className="max-w-2xl rounded-[24px] border border-white/70 bg-white/90 p-6 shadow-[0_18px_50px_-36px_rgba(15,23,42,0.24)] backdrop-blur">
                <p className="eyebrow text-slate-600">{contentTypeLabel[lesson.contentType]}</p>
                <p className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-slate-950">{lesson.title}</p>
                <p className="mt-4 text-sm leading-7 text-slate-600">{lesson.content || lesson.excerpt}</p>
              </div>
            </div>
          ) : (
            <div className="flex aspect-video items-end bg-[linear-gradient(145deg,#0f172a_0%,#1e293b_48%,#334155_100%)] p-6">
              <div className="max-w-2xl rounded-[24px] border border-white/10 bg-white/10 p-6 text-white shadow-[0_18px_50px_-36px_rgba(15,23,42,0.5)] backdrop-blur">
                ...
                <p className="mt-3 text-3xl font-semibold tracking-[-0.04em]">{lesson.title}</p>
        ...
        <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-[-0.04em] text-slate-950 sm:text-[2rem]">
              {lesson.title}
            </h1>
          ...
          {showExternalAction ? (
            <a
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 text-slate-700 transition hover:border-slate-300 hover:bg-white"
