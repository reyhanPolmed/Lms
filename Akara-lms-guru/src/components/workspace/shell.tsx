...
export function WorkspaceShell({ children }: { children: ReactNode }) {
  ...
  return (
    <main className="relative h-dvh overflow-hidden bg-[#f6f3ff] p-2.5 text-[var(--page-ink)]">
      <div className="grid h-full min-h-0 gap-2 overflow-hidden rounded-[30px] border border-[rgba(113,94,215,0.12)] bg-white/60 shadow-[0_24px_80px_rgba(101,91,199,0.16)] xl:grid-cols-[180px_minmax(0,1fr)] 2xl:grid-cols-[196px_minmax(0,1fr)]">
        <aside className="panel-surface hidden h-full overflow-hidden px-3 py-3 xl:flex xl:flex-col">
          <div className="mb-4 flex flex-col items-center">
            ...
            <p className="mt-3 text-[13px] font-semibold uppercase tracking-[0.24em] text-[#7e77ad]">
              Akara LMS
            </p>
            <p className="text-[15px] font-bold text-[#222a4a]">Teacher Hub</p>
          </div>

          <div className="flex min-h-0 flex-1 flex-col justify-between">
            {navGroups.map((group, idx) => (
              <nav key={idx} className="space-y-1.5 text-[13px] font-medium">
                {group.map((item) => {
                  ...
                  if (item.label === "Keluar") {
                    return (
                      <button
                        ...
                        className="flex w-full items-center gap-3 rounded-[14px] px-3.5 py-3 transition text-[#5b6484] hover:bg-[#ffeef1] hover:text-[#e04562]"
                      >
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={[
                        "flex items-center gap-3 rounded-[14px] px-3.5 py-3 transition",
                        active
                          ? "bg-[#f0edff] font-semibold text-[#5f52d4] shadow-[0_8px_18px_rgba(109,93,252,.08)]"
                          : "text-[#5b6484] hover:bg-[#f7f5ff]",
                      ].join(" ")}
                    >
                      <Icon className="h-5 w-5" />
                      <span>{item.label}</span>
