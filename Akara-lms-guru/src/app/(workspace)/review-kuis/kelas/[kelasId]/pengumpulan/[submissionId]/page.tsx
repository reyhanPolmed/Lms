...
export default function QuizSubmissionDetailPage({
  ...
  params: Promise<{ kelasId: string; submissionId: string }>;
}) {
  const { kelasId, submissionId } = use(params);
        ...
        {loadingDetail ? (
          ...
          <div className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-[12px] text-[#6a7393]">
                    {detail.className} • {detail.courseTitle} • Attempt {detail.attemptNumber}
                  </p>
                </div>
            ...
            {detail.attemptHistory.length > 1 ? (
              <div className="mt-3 rounded-[18px] border border-[rgba(113,94,215,0.10)] bg-white p-4">
                <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-[#8a92b6]">
                  Riwayat Attempt
                </p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {detail.attemptHistory.map((history) => (
                    <Link
                      key={history.id}
                      href={`/review-kuis/kelas/${encodeURIComponent(className)}/pengumpulan/${history.id}`}
                      className={`rounded-[12px] border px-3 py-2 text-[12px] font-semibold transition-colors ${
                        history.id === submissionId
                          ? "border-[#715ed7] bg-[#f4f2ff] text-[#5f52bf]"
                          : "border-[rgba(113,94,215,0.12)] bg-white text-[#6c7399] hover:border-[#715ed7] hover:text-[#715ed7]"
                      }`}
                    >
                      Attempt {history.attemptNumber} • {history.score ?? "—"}
                    </Link>
                  ))}
                </div>
              </div>
